...
import type { TimerMode, PomodoroSettings, ChatMessage, TodoItem } from '@/types/pomodoro';
import {
  DEFAULT_SETTINGS,
  INITIAL_TODOS,
  DUMMY_CHATS_POOL,
...
export function usePomodoroTimer() {
  ...
  useEffect(() => {
    ...
    if (isActive && timeLeft > 0) {
      ...
      chatInterval = setInterval(() => {
        if (Math.random() > 0.35) {
          const randomChat =
            DUMMY_CHATS_POOL[Math.floor(Math.random() * DUMMY_CHATS_POOL.length)];
          setChatLog((prev) => [
            ...prev.slice(-25),
  ...
  const changeMode = (newMode: TimerMode) => {
    ...
    const announceMessages: Record<TimerMode, string> = {
      ...
      shortBreak: 'Waktu Zatsudan/Rehat sebentar! ☕ Regangkan badan & minum air dulu yuk~',
